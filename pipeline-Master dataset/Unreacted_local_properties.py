...
output_folder_data = "data/local/unreacted/data"
output_folder_image = "data/local/unreacted/images"
...
for filename in sorted(os.listdir(input_folder)):
    ...
    # Define lower/upper bounds for RED in BGR
    lower_red = np.array([100, 0, 0], dtype=np.uint8)   # low B, low G, high-ish R
    upper_red = np.array([255, 100, 100], dtype=np.uint8)  # allow some B,G noise, max R
    ...
    red_only_gray = cv2.cvtColor(red_only, cv2.COLOR_RGB2GRAY)

    # Label connected regions
    s = np.ones((3, 3), dtype=int)
    label_mask, num_labels = ndimage.label(red_only_gray, structure=s)
    ...
    wb = Workbook()
    ws = wb.active
    ws.title = "Unreacted Measurements"
    ...
    excel_output_path = os.path.join(output_folder_data, f"unreacted_property_{image_name_no_ext}.xlsx")
    wb.save(excel_output_path)
    print(f"Saved Excel: unreacted_property_{image_name_no_ext}.xlsx")
"""
    # --- Generate marked image ---
    regions = regionprops(label_mask)
    fig, ax = plt.subplots()
    ax.imshow(red_only_gray, cmap=plt.cm.gray)
